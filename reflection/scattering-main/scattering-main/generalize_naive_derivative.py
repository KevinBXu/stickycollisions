...
def psi(x, index=None):
    ...
    if even:
        if x < min_x:
            momentum = 1j * k
            return np.exp(momentum * x) + np.exp(-momentum * x)
...
bpsi2 = [np.abs(psi(min_x + i * dx)) ** 2 for i in range(-int(5 / dx), 0)]

after = [min_x + i * dx for i in range(int((max_x - min_x) / dx) + 1)]
apsi2 = [np.abs(psi(min_x + i * dx, index=i)) ** 2 for i in range(int((max_x - min_x) / dx) + 1)]
