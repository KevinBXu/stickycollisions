...
def generate_spectrum(num_resonances, mean_spacing, mean_coupling_strength, sector_coupling_strength, num_sectors = 1, num_open_channels = 1):
    ...
    for i in range(num_sectors):
        ...
        energy_GOE_mat[0] = -mean_spacing * num_resonances / 2 + mean_spacing * rng.uniform(0, 1)
        ...
        sectors.append(energy_GOE_mat)

        # generate coupling matrix
        W_mat = np.zeros(num_resonances)
        coupling = mean_coupling_strength.get() if i == 0 else sector_coupling_strength.get()
        for mu in range(num_resonances):
            W_mat[mu] = np.array(rng.normal(0, coupling))

        sector_couplings.append(W_mat)

    energy_GOE_mat = np.concatenate(sectors)
    W_mat = np.concatenate(sector_couplings)
